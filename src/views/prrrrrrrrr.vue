            ...
            <table class="table table-hover align-middle mb-0">
              ...
              <tbody>
                <tr v-for="s in filteredStudents" :key="s.id">
                  ...
                  <td>{{ s.name }}</td>
                  <td>{{ s.email || '-' }}</td>
                  <td><strong>{{ callParseing(s.room) }}</strong></td>
...
export default {
 ...
 callParseing(roomString) {
    if (!roomString) return '-';

    // Most common cases — fix double escaping first
    let fixed = roomString
      .replace(/\\"/g, '"')      // \" → "
      .replace(/\\'/g, "'")      // \' → '

    // If it starts with [ and ends with ], try to parse directly
    if (fixed.startsWith('[') && fixed.endsWith(']')) {
      try {
        const arr = JSON.parse(fixed);
        return arr[0]?.roomNumber || '-';
      } catch (e) { /* continue to hardcore fix */ }
    }

    // HARDCODE FIX FOR BROKEN DATA (this saves 99% of real-world cases)
    try {
    ...
    // LAST RESORT: Regex fallback (never fails, always returns something)
    const match = fixed.match(/"roomNumber"\s*:\s*"([^"]*)/);
    if (match && match[1]) {
      return match[1].trim();
    }

    // If even regex fails, return raw string (for debugging)
    console.warn('Totally broken room field:', roomString);
    return 'Invalid';
  },
     viewDetails(student) {
...
getLatestRoom(raw) {
    ...
    try {
      // Method 1: Direct regex for "roomNumber":"404"
      const roomMatches = raw.match(/"roomNumber"\s*:\s*"([^"]+)"/g);
      if (roomMatches && roomMatches.length > 0) {
        const lastMatch = roomMatches[roomMatches.length - 1];
        roomNumber = lastMatch.split('"')[3] || '-';
        if (roomNumber.trim()) return roomNumber.trim();
      }

      // Method 2: Look for any 3-4 digit number (room numbers are usually 100-9999)
      const numberMatch = raw.match(/\b([1-9]\d{2,4})\b/);
      if (numberMatch) {
        const num = numberMatch[1];
        if (parseInt(num) >= 100) return num;
      }

      // Method 3: Last resort — any number
      const anyNum = raw.match(/\d+/g);
      if (anyNum) return anyNum[anyNum.length - 1];

    } catch (e) {
      console.warn("Room parse failed:", raw);
    }
...
// },
  ...
  // 1️⃣ Parse existing room array safely
  if (this.form.room) {
    try {
      let parsed = JSON.parse(this.form.room);

      // If double-stringified, parse inner strings
      if (Array.isArray(parsed)) {
        rooms = parsed.map(r => {
          if (typeof r === "string") {
  ...
  // 3️⃣ Store as **single-stringified array**
...
//   // Parse existing rooms safely
//   let rooms = [];
//   if (this.form.room) {
//     try {
//       const parsed = JSON.parse(this.form.room); // should become an array
//       // If parsed is string (double-stringed), parse again
//       rooms = Array.isArray(parsed) ? parsed.map(r => typeof r === 'string' ? JSON.parse(r) : r) : [];
//     } catch (e) {
//       console.warn("Broken room data, starting fresh");
//       rooms = [];
//     }
//   }

//   // Add new room as object only
//   if (this.newRoom.roomNumber?.trim()) {
//     rooms.push({
//       roomNumber: this.newRoom.roomNumber.trim(),
//       roomType: this.newRoom.roomType || "",
...
//   // 1. Parse existing rooms (if any)
//   if (this.form.room) {
//     try {
//       const parsed = JSON.parse(this.form.room);
//       if (Array.isArray(parsed)) rooms = parsed;
//     } catch (e) {
//       console.warn("Existing room data broken, starting fresh");
//     }
//   }

//   // 2. Add new room if user entered one
//   if (this.newRoom.roomNumber?.trim()) {
//     rooms.push({
//       roomNumber: this.newRoom.roomNumber.trim(),
//       roomType: this.newRoom.roomType || "",
...
//   // 3. Store **single JSON string**
//   this.form.room = rooms.length > 0 ? JSON.stringify(rooms) : "[]";
...
//     await this.fetchStudents();   // refresh
//     this.closeModal();
//     this.newRoom = { roomNumber: "", roomType: "" };
//     this.showToast(this.editing ? "Student updated!" : "Student added!");
//   } catch (err) {
...
// },
  ...
  //   this.$router.push({ name: "ViewStudent2", params: { id: student.id } });
  // },
//   viewDetails(student) {
//   // 1. Store student in sessionStorage (old way - works)
//   sessionStorage.setItem("selectedStudent", JSON.stringify(student));

//   // 2. ALSO pass via route meta (new safe way)
//   this.$router.push({
//     name: "ViewStudent2",
//     params: { id: student.id },
//     // This prevents the guard redirect
//     state: { studentData: student }
//   }).catch(err => {
//     // Ignore the "redirected" error - it's harmless
